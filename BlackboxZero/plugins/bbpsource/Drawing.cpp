...
void DrawBBPager(HWND hwnd)
{
	...
	HFONT font = CreateFont(desktop.fontSize, 0, 0, 0, FW_NORMAL, false, false, false, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY, DEFAULT_PITCH, desktop.fontFace);
		...
		do 
		{
			...
			else 
			{
				...
				if (desktop.numbers) 
				{
					...
					DrawText(buf, desktopNumber, strlen(desktopNumber), &r, DT_CENTER | DT_VCENTER | DT_SINGLELINE | DT_WORD_ELLIPSIS);
				}
			}
			i++;
		}
	...
	if (desktop.windows)
	{
		...
		winList.clear();
		EnumWindows(CheckTaskEnumProc, 0); // ... this function which passes HWNDs to CheckTaskEnumProc callback procedure
