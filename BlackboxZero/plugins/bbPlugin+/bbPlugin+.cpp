...
n_menu * BBP_n_windowmenu(struct plugin_info *PI, n_menu *m)
{
	...
	n_menuitem_bol(R, "Always On Top", "alwaysOnTop",  PI->alwaysOnTop);
	if (false == PI->alwaysOnTop)
		n_menuitem_bol(R, "Raise on DeskClick", "clickRaise", PI->clickRaise);
	...
	{
		n_menuitem_int(R, "Alpha Value", "alpha.value",  eightScale_down(PI->alphaValue), 0, 8);
