...
//	mySetTimer();
	...
	if(drawBorder)
			{
				r.left = r.left + (bevelWidth + borderWidth) + 1;
				r.top = r.top + (bevelWidth + borderWidth)+ 1;
				r.bottom = (r.bottom - (bevelWidth + borderWidth) - 1);
				r.right = (r.right - (bevelWidth + borderWidth) - 1);
			}
	...
	//			pBBox->HistoryPrev();
		...
		*/		InvalidateRect(hwndBBNoter, NULL, true);
		...
		case BB_BROADCAST:
		{
			...
			else if (!_strnicmp(szTemp, "@BBNoterWidth", 12))
			{ //changing the clock size
				width = atoi(szTemp + 13);
			...
			else if (!_strnicmp(szTemp, "@BBNoterHeight", 12))
			{ //changing the clock size
				height = atoi(szTemp + 13);
