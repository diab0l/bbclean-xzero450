...
LPSTR szAppName = "BBXO";		// The name of our window class, etc.
LPSTR szVersion = "BBXO v1.0";	// Used in MessageBox titlebars
...
//	mySetTimer();
	if(inSlit && hSlit)// Yes, so Let's let BBSlit know.
		SendMessage(hSlit, SLIT_ADD, NULL, (LPARAM)hwndBBXO);
	else inSlit = false;
	...
	ShowWindow(hwndBBXO, SW_SHOW);

	InvalidateRect(hwndBBXO, NULL, true);
	
	return 0;
}

//===========================================================================
//This function is used once in beginPlugin and in @BBXOReloadSettings def. found in WndProc.
...
//	KillTimer(hwndBBXO, IDT_TIMER);
//	KillTimer(hwndBBXO, IDT_ALARMTIMER);
	...
	SendMessage(hwndBlackbox, BB_UNREGISTERMESSAGE, (WPARAM)hwndBBXO, (LPARAM)msgs);
	if(inSlit && hSlit)
		SendMessage(hSlit, SLIT_REMOVE, NULL, (LPARAM)hwndBBXO);
	// Destroy our window...
	DestroyWindow(hwndBBXO);
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	{		
		...
		case WM_PAINT:
		{
			...
			if (fullTrans && (dwId == VER_PLATFORM_WIN32_NT)&&(dwMajorVer > 4))
				drawRect(hdc,TRANS,r,NULL,NULL);
				//{
				...
				//}
					...
					if (!strcmp(bitmapFile,".none")==0) drawRect(hdc,BMP,r,bitmapFile,NULL);
				/*		{
			...
			// the second background is painted
					...
					if ((!(fullTrans && (dwId == VER_PLATFORM_WIN32_NT)&&(dwMajorVer > 4)))||(inSlit))
					{  drawRect(hdc,TOOLBAR,r,NULL,NULL);
	...
	plane[1].bottom = r.top + (r.bottom - r.top)/3;
	plane[1].top = r.top;
	plane[1].left = r.left;
	plane[1].right = r.left + (r.right - r.left)/3;

	plane[2].bottom = r.bottom - (r.bottom - r.top)/3;
	plane[2].top = r.top + (r.bottom - r.top)/3;
	plane[2].left = r.left;
	plane[2].right = r.left + (r.right - r.left)/3;

	plane[3].bottom = r.bottom;
	plane[3].top = r.bottom - (r.bottom - r.top)/3;
	plane[3].left = r.left;
	plane[3].right = r.left + (r.right - r.left)/3;
	
	plane[4].bottom = r.top + (r.bottom - r.top)/3;
	plane[4].top = r.top;
	plane[4].left = r.left + (r.right - r.left)/3;
	plane[4].right = r.right - (r.right - r.left)/3;

	plane[5].bottom = r.bottom - (r.bottom - r.top)/3;
	plane[5].top = r.top + (r.bottom - r.top)/3;
	plane[5].left = r.left + (r.right - r.left)/3;
	plane[5].right = r.right - (r.right - r.left)/3;

	plane[6].bottom = r.bottom;
	plane[6].top = r.bottom - (r.bottom - r.top)/3;
	plane[6].left = r.left + (r.right - r.left)/3;
	plane[6].right = r.right - (r.right - r.left)/3;

	plane[7].bottom = r.top + (r.bottom - r.top)/3;
	plane[7].top = r.top;
	plane[7].left = r.right - (r.right - r.left)/3;
	plane[7].right = r.right;

	plane[8].bottom = r.bottom - (r.bottom - r.top)/3;
	plane[8].top = r.top + (r.bottom - r.top)/3;
	plane[8].left = r.right - (r.right - r.left)/3;
	plane[8].right = r.right;

	plane[9].bottom = r.bottom;
	plane[9].top = r.bottom - (r.bottom - r.top)/3;
	plane[9].left = r.right - (r.right - r.left)/3;
	plane[9].right = r.right;


	for (int i=1;i<=9;i++) 
		(i%2 == 0)?drawRect(hdc,WINLABEL,plane[i],NULL,NULL):drawRect(hdc,LABEL,plane[i],NULL,NULL);
	
	if (dRes==0){

	HPEN hPen;
    hPen = CreatePen(PS_SOLID,2,clockfontColor);
	SelectObject(hdc,hPen);
  
	
	MoveToEx(hdc,r.left + (r.right - r.left)/3,r.top,NULL);
	LineTo(hdc,r.left + (r.right - r.left)/3,r.bottom);

	MoveToEx(hdc,r.right - (r.right - r.left)/3,r.top,NULL);
	LineTo(hdc,r.right - (r.right - r.left)/3,r.bottom);
	
	MoveToEx(hdc,r.left,r.top + (r.bottom - r.top)/3,NULL);
	LineTo(hdc,r.right,r.top + (r.bottom - r.top)/3);

	MoveToEx(hdc,r.left,r.bottom - (r.bottom - r.top)/3,NULL);
	LineTo(hdc,r.right,r.bottom - (r.bottom - r.top)/3);
	
	for (int i=1;i<=9;i++){ 
		if (part[i]==1) drawRect(hdc,XDRAW,plane[i],NULL,NULL); //drawPart(hdc,plane[i],true);
		if (part[i]==2) drawRect(hdc,ODRAW,plane[i],NULL,NULL);}//drawPart(hdc,plane[i],false);}
	//drawPart(hdc,plane[2],false);
	//drawPart(hdc,plane[3],true);
	//Ellipse(hdc,10,20,50,30);
	SelectObject(hdc,hPen);
	DeleteObject(hPen);
...
//if (dRes>0)
	{				HGDIOBJ otherfont = CreateFont(18, 
					0, 0,  0, FW_NORMAL,
						false, false, false,
						DEFAULT_CHARSET,
						OUT_DEFAULT_PRECIS,
				...
				SetTextColor(hdc,0xfffff);
	    
			switch (dRes)
			{	
				case 1:
					DrawText(hdc,"X Wins",-1,&r,DT_VCENTER | DT_CENTER | DT_SINGLELINE);
					break;
				case 2:
					DrawText(hdc,"O Wins",-1,&r,DT_VCENTER | DT_CENTER | DT_SINGLELINE);
					break;
				case 3:
					DrawText(hdc,"Draw",-1,&r,DT_VCENTER | DT_CENTER | DT_SINGLELINE);
					break;
			}
	...
	*/			//Paint to the screen
		...
		case BB_BROADCAST:
		{
			...
			{
				// Hide window...
				ShowWindow( hwndBBXO, SW_HIDE);
			}
			else if (!_stricmp(szTemp, "@BBXOAbout"))
			{
				sprintf(szTemp, "%s\n\n%s ©2004 %s\n\n%s",
			...
			else if (!_stricmp(szTemp, "@BBXOPluginToggle"))
			{
				// Hide window...
				pluginToggle = !pluginToggle;
			}
			else if (!_stricmp(szTemp, "@BBXOOnTop"))
			{
				// Always on top...
				alwaysOnTop = !alwaysOnTop;

				if (alwaysOnTop && !inSlit) SetWindowPos(hwndBBXO, HWND_TOPMOST, 0, 0, 0, 0, SWP_NOACTIVATE | SWP_NOSIZE | SWP_NOMOVE);
				else SetWindowPos(hwndBBXO, HWND_NOTOPMOST, 0, 0, 0, 0, SWP_NOACTIVATE | SWP_NOSIZE | SWP_NOMOVE);
			}
			else if (!_stricmp(szTemp, "@BBXOSlit"))
			{
			...
			else if (!_stricmp(szTemp, "@BBXOTransparent"))
			{
			...
			else if (!_stricmp(szTemp, "@BBXOFullTrans"))
			{
			...
			else if (!_stricmp(szTemp, "@BBXOSnapToEdge"))
			{
				// Set the snapWindow attributes to the window
				snapWindow = !snapWindow;
			}
			else if (!_stricmp(szTemp, "@BBXODrawBorder"))
			{
				drawBorder = !drawBorder;
				InvalidateRect(hwndBBXO, NULL, true);
			}
			else if (!_stricmp(szTemp, "@BBXODrawMode"))
			{
				drawMode = !drawMode;
				InvalidateRect(hwndBBXO, NULL, true);
			}
			else if (!_stricmp(szTemp, "@BBXO24"))
			{
				h24format = !h24format;
				InvalidateRect(hwndBBXO, NULL, true);
			}
			else if (!_stricmp(szTemp, "@BBXOEnableAlarms"))
			{
				alarms = !alarms;
				InvalidateRect(hwndBBXO, NULL, true);
			}
			else if (!_stricmp(szTemp, "@BBXORemove0"))
			{
				remove_zero = !remove_zero;
				InvalidateRect(hwndBBXO, NULL, true);
			}
			else if (!_stricmp(szTemp, "@BBXOReset"))
			{
					for(int i=1;i<10;i++) part[i]=0;
				InvalidateRect(hwndBBXO, NULL, true);
			}
			else if (!_stricmp(szTemp, "@BBXONoBitmap"))
			{
				noBitmap = true;
			...
			else if (!_stricmp(szTemp, "@BBXOStyleLabel"))
			{
			...
			else if (!_stricmp(szTemp, "@BBXOStyleToolbar"))
			{
				// Set the label attributes to the window style
			...
			else if (!_stricmp(szTemp, "@BBXOStyleButton"))
			{
				// Set the label attributes to the window style
			...
			else if (!_stricmp(szTemp, "@BBXOStyleButtonPr"))
			{
				// Set the label attributes to the window style
			...
			else if (!_stricmp(szTemp, "@BBXOStyleWindowLabel"))
			{
			...
			else if (!_stricmp(szTemp, "@BBXOStyleClock"))
			{
			...
			else if (!_stricmp(szTemp, "@BBXOReloadSettings"))
			{
				
				{
					...
					if ( alwaysOnTop) SetWindowPos( hwndBBXO, HWND_TOPMOST, xpos, ypos, width, height, SWP_NOACTIVATE);
					else SetWindowPos( hwndBBXO, HWND_NOTOPMOST, xpos, ypos, width, height, SWP_NOACTIVATE);
			...
			else if (!_stricmp(szTemp, "@BBXOSaveSettings"))
			{
				WriteRCSettings();
			}
			else if (!_strnicmp(szTemp, "@BBXOMode", 15))
			{
				drawMode = atoi(szTemp + 16);
				
				InvalidateRect(hwndBBXO, NULL, true);
			}
			else if (!_strnicmp(szTemp, "@BBXOFontSize", 19))
			{
				fontSize = atoi(szTemp + 20);
				
				InvalidateRect(hwndBBXO, NULL, true);
			}
			else if (!_strnicmp(szTemp, "@BBXOClockFormat", 22))
			{
				strcpy(clockformat,szTemp + 24);
				
				InvalidateRect(hwndBBXO, NULL, true);
			}
			else if (!_strnicmp(szTemp, "@BBXOWidth", 9))
			{ //changing the clock size
				width = atoi(szTemp + 10);
			...
			else if (!_strnicmp(szTemp, "@BBXOHeight", 10))
			{ //changing the clock size
				height = atoi(szTemp + 11);
			...
			else if (!_strnicmp(szTemp, "@BBXOSetTransparent", 24))
			{
			...
			else if (!_strnicmp(szTemp, "@BBXOSetBitmap", 20))
			{
			...
			else if (!_strnicmp(szTemp, "@BBXOMessage", 18))
			{
			...
			else if (!_strnicmp(szTemp, "@BBXOPlayWav", 18))
			{
				strcpy(szTemp,szTemp + 20);
				PlaySound(szTemp, NULL, SND_FILENAME | SND_ASYNC);
							
			}
			else if (!_strnicmp(szTemp, "@BBXODatePossition", 24))
			{
				text_pos = atoi(szTemp + 25);
			
				InvalidateRect(hwndBBXO, NULL, true);
			}
			else if (!_stricmp(szTemp, "@BBXOLoadBitmap"))
			{
...
//			ofn.lpstrDefExt = defaultextension;	
		...
		case WM_DISPLAYCHANGE:
		{
			if(!inSlit || !hSlit)
			{
				...
				MoveWindow(hwndBBXO, xpos, ypos, width, height, true);
			}
		}
		break;
	...
	/*	case WM_LBUTTONUP: 
		{
			//	mag = !mag;
			//	InvalidateRect(hwndBBXO, NULL, false);
		}
		break;
	*/	
		// ==========
       ...
       //-------------------------------------------------
		...
		case WM_LBUTTONUP: 
		{
	//	if (dRes >0) {dRes=0; InvalidateRect(hwndBBXO, NULL, false);}
	//	else
		if (click) {InvalidateRect(hwndBBXO, NULL, false); reset = true;  click = false;}
		else if (reset)	{for(int i=1;i<10;i++) part[i]=0;InvalidateRect(hwndBBXO, NULL, false);reset=false;}
		else{

		POINT pos;
		GetCursorPos(&pos);	
		RECT windowRect;
		GetWindowRect(hwndBBXO,&windowRect);
		pos.x -= windowRect.left;
		pos.y -= windowRect.top;

//		if (whosTurn == 1) {
		for (int i=1;i<10;i++)
			if	(PtInRect(&plane[i],pos)) {
				if (part[i]==0){
				part[i]=1;
			//	InvalidateRect(hwndBBXO, NULL, false);
				if (getResult(1)==1) {dRes = 1;click = true;}
				else if (getResult(2)==1) {dRes = 2;click = true;}
				else if (getResult(2)==2) {dRes = 3;click = true;}	
				else{
				part[getMove()]=2;
			//	InvalidateRect(hwndBBXO, NULL, false);
					if (getResult(1)==1) {dRes = 1;click = true;}
		...
		InvalidateRect(hwndBBXO, NULL, false);
	/*	if (getResult(1)==1) {MessageBox(hwndBBXO,"X Wins","Result",MB_OK | MB_ICONERROR | MB_TOPMOST);reset = true;}
		else if (getResult(2)==1) {MessageBox(hwndBBXO,"O Wins","Result",MB_OK | MB_ICONERROR | MB_TOPMOST);reset = true;}
		else if (getResult(2)==2) {MessageBox(hwndBBXO,"Draw","Result",MB_OK | MB_ICONERROR | MB_TOPMOST);reset = true;}	
	//	}
		/*else 
		{
			for (int i=1;i<10;i++)
			if	(PtInRect(&plane[i],pos)) {part[i]=2;InvalidateRect(hwndBBXO, NULL, false); whosTurn = (whosTurn + 1)%2;}
		
			if (getResult(2)) {MessageBox(hwndBBXO,"O Wins","Result",MB_OK | MB_ICONERROR | MB_TOPMOST);reset = true; }
			
		}*/
		
		//if (whosTurn == 0) 
//		{part[getMove()]=2;InvalidateRect(hwndBBXO, NULL, false);whosTurn = (whosTurn + 1)%2;}
	...
	/*	if (!reset){
		...
		if (draw) {MessageBox(hwndBBXO,"Draw","Result",MB_OK | MB_ICONERROR | MB_TOPMOST);reset = true; }
			}*/}
	...
	/*	POINT pos;
		...
		pos.y -= windowRect.top;
		for (int i=1;i<10;i++)
			if	(PtInRect(&plane[i],pos)) {part[i]=2;InvalidateRect(hwndBBXO, NULL, false);}
		
			if (getResult(2)) {MessageBox(hwndBBXO,"O Wins","Result",MB_OK | MB_ICONERROR | MB_TOPMOST);for(int i=1;i<10;i++) part[i]=0; InvalidateRect(hwndBBXO, NULL, false);}
...
//			magStart = false;
	...
	xpos = ReadInt(rcpath, "bbxo.x:", 10);
	ypos = ReadInt(rcpath, "bbxo.y:", 10);

	width = ReadInt(rcpath, "bbxo.width:", 100);
	height = ReadInt(rcpath, "bbxo.height:", 50);

	text_pos = ReadInt(rcpath, "bbxo.date.possition:", 0);
	if ((text_pos<0) ||(text_pos>4)) text_pos = 0; 

	alpha = ReadInt(rcpath, "bbxo.alpha:", 160);
	if(alpha > 255) alpha = 255;
	
	if(ReadString(rcpath, "bbxo.inSlit:", NULL) == NULL) wantInSlit = true;
	else wantInSlit = ReadBool(rcpath, "bbxo.inSlit:", true);
	
	alwaysOnTop = ReadBool(rcpath, "bbxo.alwaysOnTop:", true);
	
	drawBorder = ReadBool(rcpath, "bbxo.drawBorder:", true);
	drawMode = ReadInt(rcpath, "bbxo.drawMode:", HORIZONTAL);
	remove_zero = ReadBool(rcpath, "bbxo.remove.0:", false);
	h24format = ReadBool(rcpath, "bbxo.24hFormat:", false);
	showSeconds = ReadBool(rcpath, "bbxo.show.seconds:", true);
	alarms = ReadBool(rcpath, "bbxo.enabled.alarms:", false);
//	if (magStart) mag = true;

	snapWindow = ReadBool(rcpath, "bbxo.snapWindow:", true);
	transparency = ReadBool(rcpath, "bbxo.transparency:", false);
	fullTrans = ReadBool(rcpath, "bbxo.fullTrans:", false);
	fontSize = ReadInt(rcpath, "bbxo.fontSize:", 6);
	alwaysOnTop = ReadBool(rcpath, "bbxo.alwaysontop:", true);
	pluginToggle = ReadBool(rcpath, "bbxo.pluginToggle:", false);
	strcpy(windowStyle, ReadString(rcpath, "bbxo.windowStyle:", "windowlabel"));
	if(((StrStrI(windowStyle, "label") == NULL) || ((StrStrI(windowStyle, "label") != NULL) && (strlen(windowStyle) > 5))) 
		...
		strcpy(windowStyle, "windowLabel");
	strcpy(bitmapFile, ReadString(rcpath, "bbxo.bitmapFile:", ".none"));
	if (strcmp(bitmapFile,".none")==0) noBitmap = true; else noBitmap = false; 
	
	strcpy(clockformat, ReadString(rcpath, "bbxo.clockformat:", "%d %a %#H:%M"));
...
//	static char temp[8];
	...
	if (file)
	{
		...
		sprintf(szTemp, "! BBXO %s alarms file.\r\n",szInfoVersion);
		WriteFile(file, szTemp, strlen(szTemp), &retLength, NULL);
...
void WriteRCSettings()
{
	...
	if (file)
	{
		...
		sprintf(szTemp, "! BBXO %s config file.\r\n",szInfoVersion);
		WriteFile(file, szTemp, strlen(szTemp), &retLength, NULL);

		sprintf(szTemp, "!============================\r\n\r\n");
		WriteFile(file, szTemp, strlen(szTemp), &retLength, NULL);
		
		sprintf(szTemp, "bbxo.x: %d\r\n", xpos, temp);
		WriteFile(file, szTemp, strlen(szTemp), &retLength, NULL);
		
		sprintf(szTemp, "bbxo.y: %d\r\n", ypos, temp);
		WriteFile(file, szTemp, strlen(szTemp), &retLength, NULL);

		sprintf(szTemp, "bbxo.width: %d\r\n", width, temp);
		WriteFile(file, szTemp, strlen(szTemp), &retLength, NULL);

		sprintf(szTemp, "bbxo.height: %d\r\n", height, temp);
		WriteFile(file, szTemp, strlen(szTemp), &retLength, NULL);

		sprintf(szTemp, "bbxo.date.possition: %d\r\n", text_pos, temp);
		WriteFile(file, szTemp, strlen(szTemp), &retLength, NULL);
		
		sprintf(szTemp, "bbxo.windowStyle: %s\r\n", windowStyle);
		WriteFile(file, szTemp, strlen(szTemp), &retLength, NULL);

		(wantInSlit) ? strcpy(temp, "true") : strcpy(temp, "false");
		sprintf(szTemp, "bbxo.inSlit: %s\r\n", temp);
 		WriteFile(file, szTemp, strlen(szTemp), &retLength, NULL);

		(alwaysOnTop) ? strcpy(temp, "true") : strcpy(temp, "false");
		sprintf(szTemp, "bbxo.alwaysOnTop: %s\r\n", temp);
 		WriteFile(file, szTemp, strlen(szTemp), &retLength, NULL);

		(snapWindow) ? strcpy(temp, "true") : strcpy(temp, "false");
		sprintf(szTemp, "bbxo.snapWindow: %s\r\n", temp);
 		WriteFile(file, szTemp, strlen(szTemp), &retLength, NULL);

		(transparency) ? strcpy(temp, "true") : strcpy(temp, "false");
		sprintf(szTemp, "bbxo.transparency: %s\r\n", temp);
 		WriteFile(file, szTemp, strlen(szTemp), &retLength, NULL);

		sprintf(szTemp, "bbxo.alpha: %d\r\n", alpha, temp);
		WriteFile(file, szTemp, strlen(szTemp), &retLength, NULL);

		(fullTrans) ? strcpy(temp, "true") : strcpy(temp, "false");
		sprintf(szTemp, "bbxo.fullTrans: %s\r\n", temp);
 		WriteFile(file, szTemp, strlen(szTemp), &retLength, NULL);

		(pluginToggle) ? strcpy(temp, "true") : strcpy(temp, "false");
		sprintf(szTemp, "bbxo.pluginToggle: %s\r\n", temp);
 		WriteFile(file, szTemp, strlen(szTemp), &retLength, NULL);

		(drawBorder) ? strcpy(temp, "true") : strcpy(temp, "false");
		sprintf(szTemp, "bbxo.drawBorder: %s\r\n", temp);
 		WriteFile(file, szTemp, strlen(szTemp), &retLength, NULL);

		sprintf(szTemp, "bbxo.drawMode: %d\r\n", drawMode, temp);
		WriteFile(file, szTemp, strlen(szTemp), &retLength, NULL);

		(remove_zero) ? strcpy(temp, "true") : strcpy(temp, "false");
		sprintf(szTemp, "bbxo.remove.0: %s\r\n", temp);
 		WriteFile(file, szTemp, strlen(szTemp), &retLength, NULL);

		(showSeconds) ? strcpy(temp, "true") : strcpy(temp, "false");
		sprintf(szTemp, "bbxo.show.seconds: %s\r\n", temp);
 		WriteFile(file, szTemp, strlen(szTemp), &retLength, NULL);

		(h24format) ? strcpy(temp, "true") : strcpy(temp, "false");
		sprintf(szTemp, "bbxo.24hFormat: %s\r\n", temp);
 		WriteFile(file, szTemp, strlen(szTemp), &retLength, NULL);

		(alarms) ? strcpy(temp, "true") : strcpy(temp, "false");
		sprintf(szTemp, "bbxo.enabled.alarms: %s\r\n", temp);
 		WriteFile(file, szTemp, strlen(szTemp), &retLength, NULL);

    	sprintf(szTemp, "bbxo.fontSize: %d\r\n", fontSize, temp);
		WriteFile(file, szTemp, strlen(szTemp), &retLength, NULL);
		
		sprintf(szTemp, "bbxo.clockformat: %s\r\n", clockformat);
		WriteFile(file, szTemp, strlen(szTemp), &retLength, NULL);

		sprintf(szTemp, "bbxo.bitmapFile: %s\r\n", bitmapFile);
		WriteFile(file, szTemp, strlen(szTemp), &retLength, NULL);
...
void mySetTimer()
{
		...
		SetTimer(hwndBBXO,		// handle to main window 
	...
	if(!inSlit)
					{
						if (transparency && (dwId == VER_PLATFORM_WIN32_NT)&&(dwMajorVer > 4))
						{
							if (fullTrans && (dwId == VER_PLATFORM_WIN32_NT)&&(dwMajorVer > 4))
							{
								SetWindowLong(hwndBBXO, GWL_EXSTYLE, WS_EX_TOOLWINDOW | WS_EX_LAYERED);
								BBSetLayeredWindowAttributes(hwndBBXO, 0xFF00FF, (unsigned char)alpha, LWA_COLORKEY|LWA_ALPHA);
							}
							else
							{
							SetWindowLong(hwndBBXO, GWL_EXSTYLE, WS_EX_TOOLWINDOW | WS_EX_LAYERED);
							BBSetLayeredWindowAttributes(hwndBBXO, NULL, (unsigned char)alpha, LWA_ALPHA);
							}
						}
						else if ((!transparency) && (dwId == VER_PLATFORM_WIN32_NT)&&(dwMajorVer > 4))
						{
							...
							{
								SetWindowLong(hwndBBXO, GWL_EXSTYLE, WS_EX_TOOLWINDOW | WS_EX_LAYERED);
								BBSetLayeredWindowAttributes(hwndBBXO, 0xFF00FF, (unsigned char)alpha, LWA_COLORKEY);
							}
							else
							SetWindowLong(hwndBBXO, GWL_EXSTYLE, WS_EX_TOOLWINDOW);
						}
							
					}
					else if((transparency)||(fullTrans)) SetWindowLong(hwndBBXO, GWL_EXSTYLE, WS_EX_TOOLWINDOW);
...
//	bool tempBool = false;
	...
	if(myMenu){ DelMenu(myMenu); myMenu = NULL;}
			...
			MakeMenuItem(otherSubmenu, "Draw Border", "@BBXODrawBorder", drawBorder);
	    //	MakeMenuItem(otherSubmenu, "Draw Mode", "@BBXODrawMode", drawMode);
		//	MakeMenuItem(otherSubmenu, "Show Seconds", "@BBXOShowSeconds", showSeconds);
		//	MakeMenuItem(otherSubmenu, "24h Format", "@BBXO24", h24format);
		//	MakeMenuItem(otherSubmenu, "Remove First 0", "@BBXORemove0", remove_zero);
		//	MakeMenuItem(otherSubmenu, "Enable Alarms", "@BBXOEnableAlarms", alarms);
			MakeMenuItemInt(otherSubmenu, "Width", "@BBXOWidth", width, 20, 400);
			MakeMenuItemInt(otherSubmenu, "Height", "@BBXOHeight", height, 20, 400);
		//	MakeMenuItemInt(otherSubmenu, "Font Size", "@BBXOFontSize", fontSize, 6, 50);
		//	MakeMenuItemString(otherSubmenu, "Text Format", "@BBXOClockFormat", clockformat);
			...
			MakeMenuItem(windowStyleSubmenu, "toolbar:", "@BBXOStyleToolbar", (StrStrI(windowStyle, "toolbar") != NULL));
			//tempBool = false;
			//if(StrStrI(windowStyle, "buttonnp") != NULL) tempBool = true;
			MakeMenuItem(windowStyleSubmenu, "toolbar.button:", "@BBXOStyleButton", (StrStrI(windowStyle, "buttonnp") != NULL));
			//tempBool = false;
			//if(StrStrI(windowStyle, "buttonpr") != NULL) tempBool = true;
			MakeMenuItem(windowStyleSubmenu, "toolbar.button.pressed:", "@BBXOStyleButtonPr", (StrStrI(windowStyle, "buttonpr") != NULL));
			//tempBool = false;
			//if(StrStrI(windowStyle, "label") != NULL && strlen(windowStyle) < 6) tempBool = true;
			MakeMenuItem(windowStyleSubmenu, "toolbar.label:", "@BBXOStyleLabel", (StrStrI(windowStyle, "label") != NULL && strlen(windowStyle) < 6));
			//tempBool = false;
			//if(StrStrI(windowStyle, "windowlabel") != NULL) tempBool = true;
			MakeMenuItem(windowStyleSubmenu, "toolbar.windowLabel:", "@BBXOStyleWindowLabel", (StrStrI(windowStyle, "windowlabel") != NULL));
			//tempBool = false;
			//if(StrStrI(windowStyle, "clock") != NULL) tempBool = true;
			MakeMenuItem(windowStyleSubmenu, "toolbar.clock:", "@BBXOStyleClock", (StrStrI(windowStyle, "clock") != NULL));
					
			configSubmenu = MakeMenu("Configuration");

			generalConfigSubmenu = MakeMenu("General");
			if(hSlit) MakeMenuItem(generalConfigSubmenu, "In Slit", "@BBXOSlit", wantInSlit);
			MakeMenuItem(generalConfigSubmenu, "Toggle with Plugins", "@BBXOPluginToggle", pluginToggle);
			MakeMenuItem(generalConfigSubmenu, "Always on top", "@BBXOOnTop", alwaysOnTop);
			if ((dwId == VER_PLATFORM_WIN32_NT)&&(dwMajorVer > 4))
				MakeMenuItem(generalConfigSubmenu, "Transparency", "@BBXOTransparent", transparency);
			if ((dwId == VER_PLATFORM_WIN32_NT)&&(dwMajorVer > 4))
				MakeMenuItemInt(generalConfigSubmenu, "Set Transparency", "@BBXOSetTransparent",alpha,0,255);
			if ((dwId == VER_PLATFORM_WIN32_NT)&&(dwMajorVer > 4))
				MakeMenuItem(generalConfigSubmenu, "Transparent Background", "@BBXOFullTrans", fullTrans);
			MakeMenuItem(generalConfigSubmenu, "Snap Window To Edge", "@BBXOSnapToEdge", snapWindow);

			browseSubmenu = MakeMenu("Browse");
			MakeMenuItem(browseSubmenu, "Browse...", "@BBXOLoadBitmap", false);

			bitmapSubmenu = MakeMenu("Bitmap");
			MakeSubmenu(bitmapSubmenu, browseSubmenu, "Bitmap");
			MakeMenuItem(bitmapSubmenu, "Nothing", "@BBXONoBitmap", noBitmap);

		//	modeSubmenu = MakeMenu("Draw Mode");
		//	tempBool = false;
		//	if (drawMode == 0) tempBool = true;
		//	MakeMenuItem(modeSubmenu, "Vertical", "@BBXOMode 0", (drawMode == 0));
		//	MakeMenuItem(modeSubmenu, "Horizontal", "@BBXOMode 1", (drawMode == 1));
		//	MakeMenuItem(modeSubmenu, "Just Text", "@BBXOMode 2", (drawMode == 2));

			settingsSubmenu = MakeMenu("Settings");
		//	MakeMenuItem(settingsSubmenu, "Edit Alarms", "@BBXOEditAlarmsRC", false);
			MakeMenuItem(settingsSubmenu, "Edit Settings", "@BBXOEditRC", false);
			MakeMenuItem(settingsSubmenu, "Reload Settings", "@BBXOReloadSettings", false);
			MakeMenuItem(settingsSubmenu, "Save Settings", "@BBXOSaveSettings", false);
			
		/*	dateSubmenu = MakeMenu("Date Position");
			//tempBool = false;
			//if (text_pos == 0) tempBool = true;
			MakeMenuItem(dateSubmenu, "No Date", "@BBXODatePossition 0", (text_pos == 0));
			//tempBool = false;
			//if (text_pos == 1) tempBool = true;
			MakeMenuItem(dateSubmenu, "Top", "@BBXODatePossition 1", (text_pos == 1));
			//tempBool = false;
			//if (text_pos == 2) tempBool = true;
			MakeMenuItem(dateSubmenu, "Bottom", "@BBXODatePossition 2", (text_pos == 2));
			//tempBool = false;
			//if (text_pos == 3) tempBool = true;
			MakeMenuItem(dateSubmenu, "Left", "@BBXODatePossition 3", (text_pos == 3));
			//tempBool = false;
			//if (text_pos == 4) tempBool = true;
			MakeMenuItem(dateSubmenu, "Right", "@BBXODatePossition 4", (text_pos == 4));
		*/	
			//attach defined menus together
			myMenu = MakeMenu("BBXO 1.0");
...
void drawRect(HDC &hdc, int type, RECT &re, LPCSTR path, LPCSTR text)
{
	...
	switch (type){
		...
		case XDRAW:
			{
			HPEN pPen,pPenOld;
			pPen = CreatePen(PS_SOLID, 3, clockColor);
			pPenOld = (HPEN)SelectObject(hdc, pPen);
			conRect(re,5);  
			  MoveToEx(hdc,re.left,re.top,NULL);
			  LineTo(hdc,re.right,re.bottom);
	
              MoveToEx(hdc,re.left,re.bottom,NULL);
			  LineTo(hdc,re.right,re.top);
			SelectObject(hdc, pPenOld);
			DeleteObject(pPen);
			SetTextColor(hdc,fontColor);
			DrawText(hdc,text,-1,&re,DT_SINGLELINE|DT_CENTER|DT_VCENTER);
		...
		case ODRAW:
			{
			HPEN pPen,pPenOld;
			pPen = CreatePen(PS_SOLID, 3, clockColorTo);
			pPenOld = (HPEN)SelectObject(hdc, pPen);
			conRect(re,5); 
			  Arc(hdc,re.left,re.top,re.right,re.bottom,re.left + (re.right - re.left)/2,re.top,re.left + (re.right - re.left)/2,re.top);
